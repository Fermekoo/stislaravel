...
@section('content')
    <section class="section">
        ...
        <div class="section-body">
            ...
            <div class="row">
                <div class="col-12">
                <form action="{{ route('roles.update', $modules['id']) }}" method="POST">
                @method('PUT')
				@csrf
                <div class="card">
                ...
                <div class="card">
                        <div class="card-body">
                            <table class="table table-striped table-md">
                                ...
                                @foreach($modules['menus'] as $menu)
                                    <tr>
                                        
                                        <th>
                                        @if($menu->count_childs > 0)
                                            <a href="#collapse{{$menu->id}}" data-toggle="collapse">+</a>
                                        @endif
                                        <input type="checkbox" value="{{ $menu->read }}" name="permissions[]" data-type="parent" data-id="{{$menu->id}}" {{ $menu->read_checked }} >
                                            {{ $menu->menu_name }}
                                        </th>
                                        @if($menu->count_permissions > 1)
                                            @foreach($menu->permissions as $mp)
                                                <th> <input type="checkbox" name="permissions[]" data-menu_id="{{ $menu->id }}" name="permissions[]" value="{{ $mp->name }}" {{ $mp->is_checked }}> </th>
                                            @endforeach
                                        @else
                                ...
                                <tbody id="collapse{{$menu->id}}" class="collapse">
                                    ...
                                    <tr>
                                        <td><input type="checkbox" data-type="parent" data-parent_menu="{{$menu->id}}" data-id="{{$child->id}}" {{ $child->is_checked }}> {{ $child->menu_name}}</td>
                                        @foreach($child->permissions as $cp)
                                            <th><input type="checkbox" name="permissions[]" data-parent_menu="{{$menu->id}}" data-menu_id="{{ $child->id }}" value="{{ $cp->name }}" id="" {{ $cp->is_checked }}></th>
                                        @endforeach
                                    </tr>
                                    @endforeach
                                </tbody>
