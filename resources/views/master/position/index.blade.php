...
@include('master.position._modal')
@endsection
@push('js')
<script>
    $(document).ready(function(){
       ...
       var table = $('#index-table').DataTable({
			...
			ajax: {
                type: 'POST',
                url: '{{ route("master.position.json") }}'
            },
			columns: [
        ...
        $('#index-table').on('click','.btnedit', function(){
            ...
            let id = $(this).data('id');
            let action = `{{ route('master.position.detail',':id') }}`;
            $.ajax({
                type: `GET`,
        ...
        $('#index-table').on('click', '.btndelete', function(){
            let id = $(this).data('id');
            let action = `{{ route('master.position.delete',':id') }}`;
        ...
        $('#btn-submit').click(function(e){
            ...
            $.ajax({
                type: `POST`,
                url: `{{ route('master.position.submit') }}`,
                data: form.serialize(),
                dataType: `json`,
                success: function(res) {
                    table.ajax.reload();
                ...
                error: function(xhr){
                    ...
                    if($.isEmptyObject(res) == false) {
                        ...
                        if(msg instanceof Object) {
                            if(msg.namaJabatan) {
                                $('#namaJabatan').addClass('is-invalid').after(`<div class="invalid-feedback">${msg.namaJabatan}</div>`)
                            }
                            if(msg.companyId) {
